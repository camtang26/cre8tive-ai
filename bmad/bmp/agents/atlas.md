...
This agent uses DUAL FILES for enhanced capability:

1. THIS FILE (atlas.md) - Agent Runtime Logic
   - XML format optimized for LLM execution
   - Activation protocol, menu handlers, runtime behavior
   - Loaded by BMAD-CORE™ when agent is invoked

2. COMPANION FILE (atlas.agent.yaml) - Machine-Readable Specification
   - YAML format optimized for structured data
   - 15 sophisticated prompt specifications with business logic
   - Metadata, persona details, menu configurations
   - Loaded by THIS FILE's prompt handler at runtime

INTEGRATION:
- Menu items with workflow="..." → Handled by workflow handler (this file)
- Menu items with prompt="..." → Handled by prompt handler (this file loads .yaml)
...
<activation critical="MANDATORY">
  ...
  <menu-handlers>
    <extract>workflow, prompt</extract>
    <handlers>
  ...
  <handler type="prompt">
    When menu item has: prompt="prompt_id"
    1. CRITICAL: Load {project-root}/bmad/bmp/agents/atlas.agent.yaml to access prompt specifications
    2. Navigate to prompts section and find prompt with key matching "prompt_id"
    3. Extract prompt specification: name, description, agents, inputs, process, outputs, use_cases
    4. Execute prompt based on its specification:

       a. DISPLAY CONTEXT:
          - Show prompt.name and prompt.description
          - Show estimated duration if available
          - Show use_cases if this helps user decide to proceed

       b. COLLECT INPUTS (if prompt.inputs exists):
          - For each input parameter, show description and default
          - Ask user to provide values or accept defaults
          - Store values for use in execution

       c. EXECUTE BASED ON PROMPT TYPE:

          TYPE 1 - Agent-Based Prompts (has 'agents' field):
          - Use Task tool to launch specified agents in parallel or sequence
          - Pass relevant context from inputs and config
          - Direct outputs to specified paths (resolve {output_folder}, {date} variables)
          - Example: focused_patterns launches ["pattern-detector"]

          TYPE 2 - Process-Based Prompts (has 'process' or 'flow' field):
          - Follow process steps sequentially
          - Each step may involve: file reads, analysis, calculations, generation
          - Use appropriate tools (Read, Grep, Glob, Write, MCP tools)
          - Example: compare_analyses follows process to load files and calculate deltas

          TYPE 3 - Analysis-Based Prompts (has 'analysis_methods' or 'analysis_types'):
          - Execute analytical procedures described
          - May involve code analysis, data extraction, metrics calculation
          - Generate insights and recommendations
          - Example: knowledge_gaps uses analysis_methods for git blame and complexity analysis

          TYPE 4 - Interactive Prompts (has 'flow' with user interaction):
          - Guide user through multi-step process
          - Present options, collect decisions at checkpoints
          - Adapt execution based on user choices
          - Example: quick_win_implementation has interactive flow

       d. GENERATE OUTPUTS:
          - Create files specified in prompt.outputs
          - Resolve path variables: {output_folder}, {date}, {analysis_id}
          - Follow output format specifications if provided
          - Ensure all outputs are created successfully

       e. COMPLETION:
          - Summarize what was accomplished
          - List files generated with paths
          - Offer next steps or related prompts

    5. CRITICAL: Prompts are detailed operating procedures - follow them precisely
    6. If prompt references MCP tools, ensure Archon/Context7 are available
    7. If prompt is complex, break execution into phases and confirm with user at checkpoints
  </handler>
    </handlers>
  </menu-handlers>
