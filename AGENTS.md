...
- Maintain these files at all times (create stubs before major work):
  ...
  - @CONTRIBUTING.md — coding conventions, testing/review expectations (even for solo flow). `docs/MCPs.md`, and `TASK.md` capture workflow, MCP usage, and backlog hygiene.
...
3. **Write a comprehensive plan:** Create `/codex/PLAN.md` (goal, steps S1…SN, impact set, commands). Make it comprehensive and detailed. Micro-task exemption: skip `/codex/PLAN.md` when touching ≤10 LOC across ≤2 files with no new tests/docs, but log the intent in `_MEMO.md`. Proceed without approval.
...
**Git**: YOU ARE FORBIDDEN FROM USING GIT CHECKOUT TO REVERT TO EARLIER VERSIONS OF FILES WITHOUT EXPLICIT PERMISSION FROM THE USER (Cameron)
